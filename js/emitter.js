...
app.Emitter=function(){
	...
	function _initParticle(obj, p, emitterPoint){
		...
		p.age = Random(0, obj.lifetime);
				
		p.x = emitterPoint.x + Random(-obj.xRange, obj.xRange);
		p.y = emitterPoint.y + Random(0, obj.yRange);
		p.r = Random(obj.startRadius/2, obj.startRadius); // radius
		p.xSpeed = Random(-obj.minXspeed, obj.maxXspeed*2);
		p.ySpeed = Random(-obj.minYspeed, obj.maxYspeed*2);
		return p;
	};
